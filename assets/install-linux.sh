#!/usr/bin/env bash
#
# Install newshell's icon + desktop entry on Linux so the GNOME/Ubuntu dock and
...
# running window to its .desktop file via the window's app_id — newshell sets
# that to "newshell" (slint::set_xdg_app_id), and this script's StartupWMClass
# matches it.
#
# Usage:
#   ./install-linux.sh [/path/to/newshell-binary]
# You normally don't need an argument: when run from inside a release package
# (the `newshell` binary sits next to this script) it is picked up automatically.
# In the source tree it falls back to ./target/release/newshell.
...
if [ -n "${1:-}" ]; then
    BIN="$1"
elif [ -x "$SCRIPT_DIR/newshell" ]; then
    BIN="$SCRIPT_DIR/newshell"
else
    BIN="$SCRIPT_DIR/../target/release/newshell"
fi
...
if [ ! -x "$BIN" ]; then
    echo "error: newshell binary not found: $BIN" >&2
    echo "Run this script from the extracted release folder (it sits next to the" >&2
    echo "'newshell' binary), or pass the binary path as an argument." >&2
    exit 1
fi
...
StartupNotify=true
StartupWMClass=newshell
EOF
chmod 644 "$APP_DIR/newshell.desktop"
...
echo "Installed:"
echo "  icon    -> $ICON_DIR/newshell.png"
echo "  desktop -> $APP_DIR/newshell.desktop"
echo "  exec    -> $BIN"
echo
