...
async fn run_sftp(
    ...
    if session.force_scp {
        let _ = events.send(SessionEvent::SftpStatus(t(
            "已按要求强制使用 SCP 传输模式",
            "SCP transfer mode forced by session setting",
        ).into()));
        return run_scp(handle, commands, self_tx, events).await;
    }
    ...
    if channel.request_subsystem(true, "sftp").await.is_err() {
        ...
        let _ = events.send(SessionEvent::SftpStatus(t(
            "SFTP 子系统不可用,已回退到 SCP 模式",
            "SFTP subsystem unavailable — fell back to SCP mode",
        ).into()));
        return run_scp(handle, commands, self_tx, events).await;
    }
    ...
    // short timeout; if even that fails we drop the (broken) sftp session and
    // fall back to SCP, mirroring the subsystem-unavailable path above
    // (#SCP-fallback).
    let probe = tokio::time::timeout(std::time::Duration::from_secs(8), sftp.metadata(".")).await;
    if probe.is_err() || probe.unwrap().is_err() {
        let _ = events.send(SessionEvent::SftpStatus(t(
            "SFTP 子系统握手成功但不可用,已回退到 SCP 模式",
            "SFTP subsystem opened but is unusable — fell back to SCP mode",
        ).into()));
        return run_scp(handle, commands, self_tx, events).await;
    }
    ...
    while let Some(cmd) = commands.recv().await {
        match cmd {
            ...
            SftpCommand::DownloadArchive {
                ...
                tokio::spawn(async move {
                    let n = names.len();
                    let tmp = format!("/tmp/newshell-{}.tar", Uuid::new_v4());
            ...
            SftpCommand::OpenTemp { remote, edit } => {
                ...
                let filename = sanitize_filename(&base_name(&remote));
                let tmp_dir = std::env::temp_dir().join("newshell");
...
// path: file transfer speaks the SCP wire protocol; listing / mkdir / rm / mv /
// chmod / cat use remote shell commands. The UI (`SftpCommand` / `SftpHandle`)
// is unchanged — the worker just routes to a different backend (#SCP-fallback).

/// Run a remote command and capture stdout; a non-zero exit is an error.
async fn shell_capture(
    ...
    ch.exec(true, cmd.as_bytes()).await.context("exec command")?;
    ...
    if status == 0 {
        ...
        Err(anyhow!(
            "{} ({status})",
            t("远程命令失败", "remote command failed")
        ))
    }
}

/// First line of a command's stdout, or None on failure.
async fn shell_first_line(
    handle: &client::Handle<SftpClientHandler>,
    cmd: &str,
) -> Option<String> {
    shell_capture(handle, cmd)
        .await
        .ok()
        .map(|s| s.lines().next().unwrap_or("").to_string())
}

/// Parse `ls -la` output into `RemoteEntry`s. The date column differs between
/// GNU (`--time-style=+%s` → a single epoch token) and BusyBox (human date), so
/// `have_epoch` selects the right name-offset.
fn parse_ls(out: &str, path: &str, have_epoch: bool) -> Result<Vec<RemoteEntry>> {
    ...
    for line in out.lines() {
        ...
        let size_tok = match tokens.next() {
            Some(s) => s,
            None => continue,
        };
        let size: u64 = size_tok.parse().unwrap_or(0);
        let rest: Vec<&str> = tokens.collect();
        let (modified, raw_name) = if have_epoch {
            let m = rest.first().and_then(|s| s.parse::<u32>().ok()).unwrap_or(0);
            let name = rest.get(1..).map(|r| r.join(" ")).unwrap_or_default();
            (m, name)
        } else {
            let name = if rest.len() >= 4 {
                rest[3..].join(" ")
            } else {
                rest.last().cloned().unwrap_or("").to_string()
            };
            (0u32, name)
        };
        // Strip any " -> target" symlink suffix.
...
/// Derive the low 12 POSIX permission bits from an `ls -l` permission string.
fn mode_from_perms(perms: &str) -> u32 {
    ...
    let p = &chars[1..10];
    let mut mode = 0u32;
    for (i, g) in p.chunks(3).enumerate() {
        if g.len() < 3 {
            break;
        }
        let mut v = 0u32;
...
async fn shell_list(
    ...
    let (out, have_epoch) = match shell_capture(
        handle,
        &format!("command ls -la --color=never --time-style=+%s {quoted}"),
    )
    .await
    {
        Ok(o) => (o, true),
        Err(_) => (
            shell_capture(handle, &format!("command ls -la {quoted}"))
                .await?,
            false,
        ),
    };
    parse_ls(&out, path, have_epoch)
}

/// Sub-directories of `path` only (for the tree), best-effort.
async fn shell_list_dirs_only(
    handle: &client::Handle<SftpClientHandler>,
    path: &str,
) -> Result<Vec<(String, String)>> {
    Ok(shell_list(handle, path)
        .await
        .map(|e| {
            e.into_iter()
                .filter(|x| x.is_dir)
                .map(|x| (x.name, x.full_path))
                .collect()
        })
        .unwrap_or_default())
}

/// Upload a single local file to the remote via the SCP protocol.
async fn scp_upload_file(
    handle: &client::Handle<SftpClientHandler>,
    local: &Path,
    remote_dir: &str,
    id: &str,
    ...
    let meta = tokio::fs::metadata(local).await.context("stat local file")?;
    if meta.is_dir() {
        return Err(anyhow!(t(
            "SCP 回退模式暂不支持文件夹上传,请打包为单个文件",
            "Folder upload is not supported in SCP fallback mode; pack it into a single file",
        )));
    }
    let size = meta.len();
    let name = sanitize_filename(&base_name(&local.to_string_lossy()));
    let cmd = format!("scp -t {}", sh_quote(remote_dir));
    let mut ch = handle
        .channel_open_session()
        .await
        .context("open scp channel")?;
    ch.exec(true, cmd.as_bytes()).await.context("exec scp -t")?;
    let mut buf: Vec<u8> = Vec::new();
    let mut err: String = String::new();

    // 1) remote sink sends a ready byte.
    loop {
        if !buf.is_empty() {
            break;
        }
        match ch.wait().await {
            ...
            Some(russh::ChannelMsg::ExitStatus { exit_status }) => {
                return Err(anyhow!(
                    "{} ({})",
                    t("SCP 进程退出", "SCP process exited"),
                    exit_status,
                ))
            }
            Some(russh::ChannelMsg::Close) | None => {
                return Err(anyhow!(t("SCP 通道关闭", "SCP channel closed")))
            }
            Some(_) => {}
        }
    }
    if buf.remove(0) != 0 {
        return Err(anyhow!(
            "{}: {}",
            t("SCP 上传被拒绝", "SCP upload rejected"),
            err.trim()
        ));
    }

    // 2) send the `C` control line, 3) read the ack.
    let control = format!("C0644 {size} {name}\n");
    ch.data(control.as_bytes())
        .await
        .context("scp control line")?;
    buf.clear();
    err.clear();
    loop {
        if !buf.is_empty() {
    ...
    if buf.remove(0) != 0 {
        return Err(anyhow!(
            "{}: {}",
            t("SCP 上传确认失败", "SCP upload ack failed"),
            err.trim()
        ));
    }

    // 4) stream the file.
    emit_transfer(events, id, &name, true, 0, size, 0, "");
    let mut file = tokio::fs::File::open(local).await.context("open local file")?;
    let mut sent: u64 = 0;
    ...
    loop {
        ...
        let n = file.read(&mut chunk).await.context("read local file")?;
        if n == 0 {
            break;
        }
        // write this 32 KiB chunk, draining incoming acks / window
        // adjustments if the send window is ever full.
        let mut off = 0;
        while off < n {
            let c = (n - off).min(32 * 1024);
            match ch.data(&chunk[off..off + c]).await {
                Ok(()) => off += c,
                Err(_) => match ch.wait().await {
                    Some(russh::ChannelMsg::Close) | None => {
                        return Err(anyhow!(t("SCP 通道已关闭", "SCP channel closed")))
                    }
                    Some(_) => continue,
                },
            }
        ...
        if last.elapsed() >= Duration::from_millis(150) {
            last = Instant::now();
            emit_transfer(events, id, &name, true, sent, size, 0, "");
        }
    }
    emit_transfer(events, id, &name, true, size, size, 0, "");
    // 5) EOF marker, 6) final ack (best-effort).
    ch.data(&[0u8][..]).await.context("scp eof")?;
    buf.clear();
    loop {
        if !buf.is_empty() {
            break;
...
/// Download a single remote file via the SCP protocol.
async fn scp_download_file(
    handle: &client::Handle<SftpClientHandler>,
    remote: &str,
    local_path: &str,
    name: &str,
    ...
    if shell_capture(handle, &format!("test -d {}", sh_quote(remote)))
        .await
        .is_ok()
    {
        return Err(anyhow!(t(
            "SCP 回退模式暂不支持文件夹下载,请打包为单个文件",
            "Folder download is not supported in SCP fallback mode; pack it into a single file",
        )));
    }
    ...
    let mut ch = handle
        ...
        .await
        .context("open scp channel")?;
    ch.exec(true, cmd.as_bytes()).await.context("exec scp -f")?;
    let mut buf: Vec<u8> = Vec::new();
    let mut err: String = String::new();

    // 1) we are the sink: send the ready byte.
    ch.data(&[0u8][..]).await.context("scp ready")?;
    // 2) read the `C` control line.
    loop {
        if buf.contains(&b'\n') {
            break;
        }
    ...
    if kind.starts_with('D') {
        return Err(anyhow!(t(
            "SCP 回退模式暂不支持文件夹下载",
            "Folder download is not supported in SCP fallback mode",
        )));
    }
    if !kind.starts_with('C') {
        return Err(anyhow!(
            "{}: {} | {}",
            t("SCP 下载协议异常", "Unexpected SCP message"),
            line,
            err.trim()
        ));
    }
    let size: u64 = parts.next().and_then(|s| s.parse().ok()).unwrap_or(0);
    let remote_name = parts.next().unwrap_or("").to_string();
    let fname: &str = if name.is_empty() {
        &base_name(&remote_name)
    } else {
        name
    };
    // 3) ack the `C` line, 4) read `size` bytes.
    ch.data(&[0u8][..]).await.context("scp ack")?;
    emit_transfer(events, id, fname, false, 0, size, 0, "");
    ...
    while got < size {
        ...
        let need = ((size - got) as usize).min(buf.len()).min(32 * 1024);
        let piece: Vec<u8> = buf.drain(..need).collect();
        file.write_all(&piece).await.context("write local file")?;
        got += piece.len() as u64;
        if last.elapsed() >= Duration::from_millis(150) {
            last = Instant::now();
            emit_transfer(events, id, fname, false, got, size, 0, "");
        }
    }
    emit_transfer(events, id, fname, false, size, size, 0, "");
    // 5) EOF ack.
    ch.data(&[0u8][..]).await.context("scp eof ack")?;
    let _ = file.flush().await;
    Ok(true)
}

/// Recursively upload a local directory over SCP (SCP fallback folder support).
/// Creates each remote directory with `mkdir -p` and `scp -t` every file (#SCP-fallback).
async fn scp_upload_dir(
    handle: &client::Handle<SftpClientHandler>,
    local: &Path,
    remote_dir: &str,
    id: &str,
    ...
    exec_remote(handle, &format!("mkdir -p {}", sh_quote(&remote_dir)))
        .await
        .map_err(|e| anyhow!("{}: {e}", t("创建远程目录失败", "mkdir remote dir failed")))?;
    let mut stack: Vec<(std::path::PathBuf, String)> = vec![(local.to_path_buf(), remote_dir)];
    while let Some((ldir, rdir)) = stack.pop() {
        ...
        let mut rd = tokio::fs::read_dir(&ldir)
            .await
            .map_err(|e| anyhow!("{}: {e}", t("读取本地目录失败", "read local dir failed")))?;
        while let Some(entry) = rd.next_entry().await? {
            let path = entry.path();
            let name = entry.file_name().to_string_lossy().into_owned();
            let rchild = format!("{}/{}", rdir.trim_end_matches('/'), name);
            let meta = entry
                .metadata()
                .await
                .map_err(|e| anyhow!("{}: {e}", t("读取文件属性失败", "stat failed")))?;
            if meta.is_dir() {
                exec_remote(handle, &format!("mkdir -p {}", sh_quote(&rchild)))
                    .await
                    .map_err(|e| anyhow!("{}: {e}", t("创建远程目录失败", "mkdir remote dir failed")))?;
                stack.push((path, rchild));
            } else {
                let _ = events.send(SessionEvent::SftpStatus(format!(
                    "{} {}...",
                    t("上传", "Uploading"),
                ...
                if !scp_upload_file(handle, &path, &rchild, id, events, cancel).await? {
                    return Ok(false);
                }
            }
        }
...
/// Recursively download a remote directory over SCP (SCP fallback folder support).
/// Lists each level with `ls` and `scp -f` every file into the matching local tree (#SCP-fallback).
async fn scp_download_dir(
    ...
    tokio::fs::create_dir_all(local_dir)
        .await
        .map_err(|e| anyhow!("{}: {e}", t("创建本地目录失败", "create local dir failed")))?;
    let entries = shell_list(handle, remote).await?;
    for e in entries {
        if cancel.load(Ordering::Relaxed) {
            return Ok(false);
        }
        ...
        if e.is_dir {
            Box::pin(scp_download_dir(handle, &e.full_path, &dest, id, events, cancel)).await?;
        } else {
            let _ = events.send(SessionEvent::SftpStatus(format!(
                "{} {}...",
                t("下载", "Downloading"),
            ...
            scp_download_file(handle, &e.full_path, &dest, &e.name, id, events, cancel).await?;
        }
    }
    Ok(true)
}

/// SCP fallback worker: mirrors `run_sftp`'s command loop but routes every
/// operation through `scp` / remote shell commands instead of the SFTP
/// subsystem.
async fn run_scp(
    handle: client::Handle<SftpClientHandler>,
    mut commands: UnboundedReceiver<SftpCommand>,
    ...
    let handle = std::sync::Arc::new(handle);
    // Guard: the SCP fallback needs a working `scp` binary on the remote. If it's
    // missing, surface a clear error instead of failing mid-transfer (#SCP-fallback).
    if shell_capture(&handle, "command -v scp").await.is_err() {
        let _ = events.send(SessionEvent::SftpError(t(
            "远程缺少 scp,无法使用 SCP 回退模式",
            "Remote has no `scp`; SCP fallback unavailable",
        )
        .to_string()));
        return Ok(());
    }
    ...
    // Resolve home and do an initial listing.
    let home = shell_first_line(&handle, "echo \"$HOME\"")
        .await
        .unwrap_or_else(|| "/".to_string());
    let home = if home.is_empty() { "/".to_string() } else { home };
    let _ = events.send(SessionEvent::SftpStatus(format!(
        "{} {}...",
        t("SCP 加载", "SCP loading"),
        home
    )));
    match shell_list(&handle, &home).await {
        Ok(entries) => {
            let _ = events.send(SessionEvent::SftpEntries {
                path: home.clone(),
                entries,
    ...
    // Tree state (mirrors run_sftp).
    let mut tree_dirs: HashMap<String, Vec<(String, String)>> = HashMap::new();
    let mut tree_expanded: std::collections::HashSet<String> = std::collections::HashSet::new();
    tree_dirs.insert(
        "/".to_string(),
        shell_list_dirs_only(&handle, "/").await.unwrap_or_default(),
    );
    tree_expanded.insert("/".to_string());
    if home != "/" {
        let mut cur = "/".to_string();
        for seg in home.trim_start_matches('/').split('/') {
            let next = format!("{}/{}", cur.trim_end_matches('/'), seg);
            tree_dirs.insert(
                next.clone(),
                shell_list_dirs_only(&handle, &next).await.unwrap_or_default(),
            );
            tree_expanded.insert(next.clone());
            cur = next;
        }
    }
    emit_tree(&tree_dirs, &tree_expanded, &events);
    ...
    while let Some(cmd) = commands.recv().await {
        match cmd {
            ...
            SftpCommand::ListDir(path) => {
                ...
                match shell_list(&handle, &path).await {
                    Ok(entries) => {
                        let _ = events.send(SessionEvent::SftpEntries {
                            path: path.clone(),
                            entries,
            ...
            SftpCommand::RefreshDir(path) => {
                ...
                for dir in expanded {
                    tree_dirs
                        .insert(dir.clone(), shell_list_dirs_only(&handle, &dir).await.unwrap_or_default());
                }
                emit_tree(&tree_dirs, &tree_expanded, &events);
            }
            SftpCommand::ToggleTreeNode(path) => {
                if tree_expanded.contains(&path) {
                    let prefix = format!("{}/", path.trim_end_matches('/'));
                    tree_expanded.retain(|p| p != &path && !p.starts_with(&prefix));
                } else if !tree_dirs.contains_key(&path) {
                    tree_dirs.insert(
                        path.clone(),
                        shell_list_dirs_only(&handle, &path).await.unwrap_or_default(),
                    );
                    tree_expanded.insert(path.clone());
                } else {
                    tree_expanded.insert(path.clone());
            ...
            SftpCommand::Download { remote, local_dir } => {
                ...
                let file_id = Uuid::new_v4().to_string();
                let cancel = Arc::new(AtomicBool::new(false));
                cancels.lock().unwrap().insert(file_id.clone(), cancel.clone());
                let cancels_done = cancels.clone();
                let id = file_id.clone();
                tokio::spawn(async move {
                    let filename = sanitize_filename(&base_name(&remote));
                    let local_path = format!("{}/{}", local_dir.trim_end_matches('/'), filename);
                    let is_dir = shell_capture(&handle, &format!("test -d {}", sh_quote(&remote)))
                        .await
                        .is_ok();
                    let _ = events.send(SessionEvent::SftpStatus(format!(
                        "{} {}...",
                    ...
                    let res = if is_dir {
                        ...
                        scp_download_dir(&handle, &remote, &local_path, &id, &events, &cancel).await
                    } else {
                        scp_download_file(&handle, &remote, &local_path, &filename, &id, &events, &cancel).await
                    };
                    match res {
                        Ok(true) => {
                            let _ = events.send(SessionEvent::SftpStatus(format!(
            ...
            SftpCommand::DownloadArchive { .. } | SftpCommand::CopyTo { .. } => {
                let _ = events.send(SessionEvent::SftpStatus(
                    t(
                        "SCP 回退模式不支持该操作",
                        "This operation is not supported in SCP fallback mode",
                    )
                    .to_string(),
                ));
            }
            SftpCommand::CancelTransfer(id) => {
                if let Some(c) = cancels.lock().unwrap().get(&id) {
            ...
            SftpCommand::Upload { local, remote_dir, .. } => {
                ...
                tokio::spawn(async move {
                    ...
                    let res = if is_dir {
                        scp_upload_dir(&handle, &local, &remote_dir, &id, &events, &cancel).await
                    } else {
                        scp_upload_file(&handle, &local, &remote_dir, &id, &events, &cancel).await
                    };
                    match res {
                        Ok(true) => {
                            let _ = events.send(SessionEvent::SftpStatus(format!(
                            ...
                            if let Ok(entries) = shell_list(&handle, &remote_dir).await {
                                let _ = events.send(SessionEvent::SftpEntries {
                                    path: remote_dir.clone(),
                                    entries,
                                });
                            }
                            let _ = events.send(SessionEvent::SftpStatus(remote_dir));
                        }
                        Ok(false) => {
                            let _ = events.send(SessionEvent::SftpStatus(format!(
                                "{}: {}",
                        ...
                        Err(e) => {
                            emit_transfer(&events, &id, &name, true, 0, 0, 2, &e.to_string());
                            let _ = events.send(SessionEvent::SftpStatus(format!(
                                "{}: {e}",
                                t("上传失败", "Upload failed")
                            )));
            ...
            SftpCommand::Delete(path) => {
                ...
                let events = events.clone();
                tokio::spawn(async move {
                    let is_dir = shell_capture(&handle, &format!("test -d {}", sh_quote(&path)))
                        .await
                        .is_ok();
                    let cmd = if is_dir {
                    ...
                    match exec_remote(&handle, &cmd).await {
                        Ok(0) => {
                            let _ = events.send(SessionEvent::SftpStatus(format!(
                                "{}: {}",
                                t("已删除", "Deleted"),
                ...
                tokio::spawn(async move {
                    ...
                    match exec_remote(&handle, &cmd).await {
                        ...
                        _ => {
                            let _ = events.send(SessionEvent::SftpStatus(format!(
                                "{}: {}",
                                t("权限更新失败", "Permission update failed"),
                                base_name(&path)
                            )));
                        }
                    }
            ...
            SftpCommand::TouchFile(path) => {
                ...
                let events = events.clone();
                tokio::spawn(async move {
                    let cmd = format!("touch {}", sh_quote(&path));
                    match exec_remote(&handle, &cmd).await {
                        Ok(0) => {
                            let _ = events.send(SessionEvent::SftpStatus(format!(
                                "{}: {}",
                                t("已创建文件", "Created file"),
                                base_name(&path)
                            )));
                        }
                        _ => {
            ...
            SftpCommand::OpenTemp { remote, edit } => {
                ...
                tokio::spawn(async move {
                    let tmp = std::env::temp_dir().join(format!("newshell-edit-{}", Uuid::new_v4()));
                    let name = sanitize_filename(&base_name(&remote));
                    let local = tmp.join(&name);
                    let id = Uuid::new_v4().to_string();
                    let cancel = Arc::new(AtomicBool::new(false));
                    match scp_download_file(&handle, &remote, &local.to_string_lossy(), &name, &id, &events, &cancel).await {
                        Ok(true) => {
                            open_with_os(local.to_string_lossy().as_ref());
                            if edit {
                                spawn_edit_watcher(
                                    self_tx,
            ...
            SftpCommand::ReadText { remote, edit } => {
                ...
                let events = events.clone();
                tokio::spawn(async move {
                    match shell_capture(&handle, &format!("cat {}", sh_quote(&remote))).await {
                        Ok(content) => {
                            let _ = events.send(SessionEvent::SftpFileText {
                                path: remote.clone(),
            ...
            SftpCommand::WriteText { remote, content } => {
                ...
                let events = events.clone();
                tokio::spawn(async move {
                    let tmp = std::env::temp_dir().join(format!("newshell-write-{}", Uuid::new_v4()));
                    let name = sanitize_filename(&base_name(&remote));
                    let local = tmp.join(&name);
                    if let Err(e) = tokio::fs::write(&local, content).await {
                        let _ = events.send(SessionEvent::SftpStatus(format!(
                            "{}: {e}",
                            t("保存失败", "Save failed")
                        )));
                    ...
                    let id = Uuid::new_v4().to_string();
                    let cancel = Arc::new(AtomicBool::new(false));
                    match scp_upload_file(&handle, &local, &parent_dir(&remote), &id, &events, &cancel).await {
                        Ok(_) => {
                            let _ = events.send(SessionEvent::SftpStatus(format!(
                                "{}: {}",
                                t("已保存", "Saved"),
...
async fn stage_remote_for_copy(
    ...
    let cleanup_root =
        std::env::temp_dir().join(format!("newshell-remote-copy-{}", Uuid::new_v4()));
