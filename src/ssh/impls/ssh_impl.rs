...
/// True when a ZMODEM init starts exactly at a chunk boundary: the previous
/// chunk's last byte was ZDLE (0x18) and this chunk opens with the frame-type
/// byte. SSH packetisation can split the two-byte signature anywhere, and a
/// split that goes undetected leaks the whole binary session to the terminal
/// (#zmodem-split).
fn zmodem_init_at_boundary(prev_tail: Option<u8>, data: &[u8]) -> bool {
    prev_tail == Some(0x18) && matches!(data.first().copied(), Some(b'B') | Some(b'C'))
}

/// Peek the ZMODEM frame type from a chunk that `contains_zmodem_init` already
/// matched. Returns `Some(0)` for ZRQINIT (remote `sz` → we receive/download),
/// `Some(1)` for ZRINIT (remote `rz` → we send/upload), or `None` if the type
/// byte can't be read — callers then fall back to the receive path.
fn zmodem_frame_type(data: &[u8]) -> Option<u8> {
    let mut i = 0;
    while i + 1 < data.len() {
        if data[i] == 0x18 {
            match data[i + 1] {
                b'B' => {
                    // Hex header: two hex digits encode the frame type.
                    if i + 3 < data.len() {
                        let hi = (data[i + 2] as char).to_digit(16);
                        let lo = (data[i + 3] as char).to_digit(16);
                        if let (Some(h), Some(l)) = (hi, lo) {
                            return Some((h * 16 + l) as u8);
                        }
                    }
                }
                b'C' => {
                    // Binary header: the type is the raw byte that follows.
                    if i + 2 < data.len() {
...
/// Find a newshell command-capture sequence (`ESC ] 697 ; <command> BEL|ST`)
...
pub enum SessionEvent {
    ...
    ResourceStats {
        ...
        /// Parsed and asserted in tests, but the live UI reads user/procs from
        /// the separate `ProcessStats` variant, so this copy is never consumed.
        #[allow(dead_code)]
        current_user: String,
        ...
        /// Remote host uptime in seconds (from `/proc/uptime`). `None` when the
        /// host can't report it (non-Linux / restricted), which the sidebar
        /// renders as "无法获取运行时间".
        ...
        Err(_) => {
            ...
            ProcessKillResult {
                ...
                message: format!(
                    "{} ({stage})",
                    t(
                        "结束进程超时，详情见 stderr 输出",
                        "Timed out; details are on stderr"
                    )
                ),
            }
        }
...
// Ciphers offered to the server.
//
// IMPORTANT (black-Synology / weak-CPU fix): the client's order decides which
// cipher OpenSSH (acting as the server) picks, and a hacked Synology almost
// always runs on an x86 without AES-NI. The old list put `chacha20-poly1305`
// and `aes256-*` FIRST, so those software-only (or slow) algorithms got
// selected and the single SSH/SFTP/ZMODEM stream was capped at a few KB/s by
// the CPU.
//
// NOTE: russh 0.49.x does NOT implement any AES-GCM cipher — only the CTR and
// CBC families plus `chacha20-poly1305` exist (the compiler error
// `cannot find value AES_128_GCM in module russh::cipher` confirms this). So we
// lead with `aes128-ctr`, the fastest hardware-accelerated choice on the AES-NI
// x86 boxes a hacked Synology usually runs on; `chacha20` stays as a middle
// fallback for ARM / no-AES-NI, and `aes256-*` drops behind it. CBC / 3DES
// remain last-resort fallbacks for legacy gear (#172). This is the same "use a
// faster cipher" tweak FinalShell / MobaXterm recommend for slow transfers.
pub(crate) const COMPAT_CIPHER: &[russh::cipher::Name] = &[
    russh::cipher::AES_128_CTR,    // fastest on AES-NI x86 (Synology)
    russh::cipher::CHACHA20_POLY1305, // middle fallback: ARM / no-AES-NI
...
async fn run_session(
    ...
    // Describe how we actually reached the host, for the persistent status line:
    //   • SSH jump host (bastion) — takes precedence; a jump connection ignores
    //     the per-session proxy, so we show the bastion's user@host:port (its IP)
    //     rather than a misleading "no proxy" (#211).
    //   • SOCKS5 / HTTP proxy — `describe` renders the scheme (socks5:// or
    //     http://) uniformly, so both kinds display the same way (#7).
    //   • otherwise a direct connection.
    let proxy_suffix = if let Some(j) = jump.as_ref() {
        // Name the bastion between "经跳板机" and "连接" (English: after "via
        // jump host") when it has one, so the status reads like
        // "经跳板机 <名称> 连接 user@host:port → target". Fall back to the plain
        // wording for an unnamed jump session.
        let via = if j.name.trim().is_empty() {
            t("经跳板机连接", "via jump host").to_string()
        } else {
            format!(
                "{}{}{}",
                t("经跳板机 ", "via jump host "),
                j.name.trim(),
                t(" 连接", "")
            )
        };
        ...
        match crate::ssh::proxy::resolve(&session.proxy) {
            ...
            None => t("未使用代理连接", "direct connection (no proxy)").to_string(),
        }
    };
    let _ = events.send(SessionEvent::Status(format!(
        "{} {}@{} -- {}",
    ...
    // Last byte of the previous channel chunk: a ZMODEM init whose ZDLE lands
    // at the end of one packet with its type byte starting the next is only
    // detectable across the boundary (#zmodem-split).
    ...
    // `UPTIME=<seconds>` rides on the fast sample so the sidebar's "已运行" line
    // updates live. `/proc/uptime`'s first field is seconds-since-boot as a float
    // (Linux). Hosts without it (non-Linux / restricted) emit an empty value, and
    // the UI then shows "无法获取运行时间". Emitted before `__DF__` so it parses in
    // the Top section, not the df block.
    const MON_CMD: &[u8] = b"PATH=/usr/bin:/bin:/usr/sbin:/sbin; export PATH; while :; do echo UPTIME=$(cut -d' ' -f1 /proc/uptime 2>/dev/null); awk '/^cpu /{print}' /proc/stat; awk '/^(MemTotal|MemAvailable|SwapTotal|SwapFree|Buffers|Cached):/{print}' /proc/meminfo; cat /proc/net/dev; echo __DF__; df -kP 2>/dev/null; echo __MSTICK__; sleep 2; done\n";
            ...
            msg = channel.wait() => {
                match msg {
                    Some(ChannelMsg::Data { data }) => {
                        ...
                        // The two-byte init signature can straddle an SSH packet
                        // boundary: ZDLE at the end of the previous chunk, the
                        // frame-type byte at the start of this one (#zmodem-split).
                        let boundary_hit = zmodem_init_at_boundary(prev_tail, &data);
                        let zmodem_hit = contains_zmodem_init(&data) || boundary_hit;
                        prev_tail = data.last().copied();
                        if !zmodem_cooldown && zmodem_hit {
                            // Reassemble the session's first bytes. On a boundary
                            // hit the ZDLE itself lived in the previous chunk (and
                            // was already rendered as a stray control char — a
                            // cosmetic artefact we accept), so prepend it for the
                            // protocol parser.
                            let mut first: Vec<u8> =
                                if boundary_hit && !contains_zmodem_init(&data) {
                            ...
                            // The frame-type byte itself may also be split across
                            // packets; wait briefly for the rest before choosing
                            // the direction (upload vs download) instead of
                            // falling through to the wrong path.
                            if zmodem_frame_type(&first).is_none() {
                                if let Ok(Some(ChannelMsg::Data { data: more })) =
                                    tokio::time::timeout(
                                    ...
                                    .await
                                {
                                    first.extend_from_slice(&more);
                                }
                            }
                            // Arm the close-frame cooldown only AFTER the transfer
                            // finishes (set below before `continue`). Arming it at
                            // detection let any transfer longer than 2 s expire it,
                            // so late close frames spawned a spurious second
                            // session (#76). The detection→transfer→continue all
                            // happen in this one iteration, so no per-iteration
                            // clearing is needed.
                            // ZRINIT (1) = remote `rz` → it wants us to send files.
                            // Pop a native multi-file picker, then ZMODEM-send.
                            // ZRQINIT (0) / unknown = remote `sz` → we receive.
                            if zmodem_frame_type(&first) == Some(1) {
                                let _ = events.send(SessionEvent::SftpStatus(t(
                                    "检测到远端上传请求,请选择要发送的文件…",
                                    "Remote upload requested — pick a file to send…",
                                ).into()));
                                // Pop a native multi-file picker on a dedicated
                                // thread (rfd is blocking) and bridge it back via
                                // the shared dialog helper (#dialog-helper).
                                match crate::dialog::pick_files_async().await {
                                    crate::dialog::PickResult::Selected(files)
                                        if !files.is_empty() =>
                                    {
                                        match crate::terminal::zmodem::send(
                                            &mut channel, &first, files, &events,
                                        )
                                        .await
                                        {
                                            Ok(()) => {}
                                            Err(e) => {
                                                tracing::warn!("zmodem send failed: {e:#}");
                                                let _ = channel.data(&ZMODEM_CANCEL[..]).await;
                                                let _ = events.send(SessionEvent::Output(format!(
                                                    "\r\n[NewShell 新の世界] {}: {e}\r\n",
                                                    t("ZMODEM 上传失败,已取消", "ZMODEM upload failed; cancelled")
                                                ).into()));
                                            }
                                        }
                                    }
                                    _ => {
                                        ...
                                        let _ = events.send(SessionEvent::Output(format!(
                                            "\r\n[NewShell 新の世界] {}\r\n",
                                            t("已取消 ZMODEM 上传", "ZMODEM upload cancelled")
                                        ).into()));
                                    }
                                }
                            } else {
                                // Default path: remote `sz` → receive into Downloads.
                                let result =
                                    crate::terminal::zmodem::receive(&mut channel, &first, &events).await;
                                match result {
                                    Ok(leftover) => {
...
fn parse_monitor_block(
    ...
    // Remote uptime seconds from `/proc/uptime` (fast sample) — `None` until a
    // parseable `UPTIME=<float>` line arrives, so a host that can't report it
    // stays None and the sidebar shows "无法获取运行时间".
    let mut uptime_secs: Option<f64> = None;
    ...
    for line in block.lines() {
        ...
        if let Some(v) = line.strip_prefix("UPTIME=") {
            // `/proc/uptime` field 1 (seconds-since-boot, float). Empty/garbage
            // (host couldn't read it) leaves `uptime_secs` None.
            let v = v.trim();
            if !v.is_empty() {
                uptime_secs = v.parse::<f64>().ok().filter(|s| *s >= 0.0);
            }
...
#[cfg(test)]
mod zmodem_detect_tests {
    use super::{contains_zmodem_init, zmodem_frame_type, zmodem_init_at_boundary};

    #[test]
    fn detects_hex_and_binary_init() {
        assert!(contains_zmodem_init(b"**\x18B00"));
        assert!(contains_zmodem_init(b"**\x18C\x01"));
        // A lone Ctrl-X (0x18) in normal output is not a transfer.
        assert!(!contains_zmodem_init(b"abc\x18def"));
        assert!(!contains_zmodem_init(b"no init here"));
    }

    #[test]
    fn detects_init_split_across_chunks() {
        // ZDLE at the end of one SSH packet, type byte at the start of the
        // next — must still count as a hit (#zmodem-split).
        let prev: &[u8] = b"shell prompt **\x18";
        let next: &[u8] = b"B01000000\r\n";
        assert!(!contains_zmodem_init(prev));
        assert!(!contains_zmodem_init(next));
        assert!(zmodem_init_at_boundary(prev.last().copied(), next));
        assert!(!zmodem_init_at_boundary(Some(b'x'), next));
        assert!(!zmodem_init_at_boundary(prev.last().copied(), b"nope"));
    }

    #[test]
    fn frame_type_picks_direction() {
        // Hex ZRQINIT ("00") → receive; hex ZRINIT ("01") → upload.
        assert_eq!(zmodem_frame_type(b"**\x18B00"), Some(0));
        assert_eq!(zmodem_frame_type(b"**\x18B01"), Some(1));
        // Binary header: the raw byte after ZBIN32 is the frame type.
        assert_eq!(zmodem_frame_type(b"**\x18C\x00"), Some(0));
        assert_eq!(zmodem_frame_type(b"**\x18C\x01"), Some(1));
        // Truncated at the type byte → undetermined (caller waits for more).
        assert_eq!(zmodem_frame_type(b"**\x18B"), None);
        assert_eq!(zmodem_frame_type(b"**\x18C"), None);
    }
}
