...
pub fn load(id: &str) -> Option<Wallpaper> {
    ...
    let buf = match id {
        ...
        "builtin:kuma" => decode_kuma()?,
        "builtin:rabbit" => decode_rabbit()?,
...
/// Bundled "Kuma" wallpaper, embedded so it ships as a built-in.
fn decode_kuma() -> Option<SharedPixelBuffer<Rgba8Pixel>> {
    const BYTES: &[u8] = include_bytes!("../../../assets/kuma.jpg");
    Some(to_buffer(image::load_from_memory(BYTES).ok()?.to_rgba8()))
}

/// Bundled "Rabbit" wallpaper, embedded so it ships as a built-in.
fn decode_rabbit() -> Option<SharedPixelBuffer<Rgba8Pixel>> {
    const BYTES: &[u8] = include_bytes!("../../../assets/Rabbit.jpg");
    Some(to_buffer(image::load_from_memory(BYTES).ok()?.to_rgba8()))
}
